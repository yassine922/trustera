...
function ButtonGroupText({ className, ...props }: React.ComponentProps<"div"> & { asChild?: boolean }) {
  return (
    <div className={cn("bg-muted flex items-center gap-2 rounded-md border px-4 text-sm font-medium", className)} {...props} />
  );
}

function ButtonGroupSeparator({ className, orientation = "vertical", ...props }: React.ComponentProps<typeof Separator>) {
  return (
    <Separator data-slot="button-group-separator" orientation={orientation}
      className={cn("bg-input relative !m-0 self-stretch data-[orientation=vertical]:h-auto", className)} {...props} />
  );
}
