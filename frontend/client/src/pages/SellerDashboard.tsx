...
export default function SellerDashboard() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f5f7fa', direction: 'rtl' }}>
      ...
      {/* ===== Sidebar ===== */}
      <div style={{ width: '220px', background: 'white', borderLeft: '1px solid #eef0f3', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px', borderBottom: '1px solid #eef0f3' }}>
          <div style={{ width: '48px', height: '48px', background: '#edf7f0', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', marginBottom: '10px' }}>🏪</div>
          <div style={{ fontWeight: 800, fontSize: '15px' }}>{user?.name}</div>
          <div style={{ fontSize: '12px', color: '#1a7c2e', marginTop: '2px' }}>✅ بائع موثّق</div>
        </div>
        ...
        <nav style={{ padding: '10px 0', flex: 1 }}>
          {navItems.map(item => (
            <button key={item.key} onClick={() => setSection(item.key)} style={{
              ...
              fontFamily: 'Cairo,sans-serif', fontSize: '13px', fontWeight: section === item.key ? 700 : 500,
          ...
          <button onClick={() => showPage('categories')} style={{
            width: '100%', padding: '11px 20px', border: 'none', textAlign: 'right',
            ...
            <div style={{ background: 'white', borderRadius: '14px', padding: '24px', border: '1px solid #eef0f3', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <div
                  ...
                  style={{ border: '2px dashed #dde1e7', borderRadius: '10px', padding: '20px', textAlign: 'center', cursor: 'pointer', background: '#fafafa', transition: 'all 0.2s' }}
                >
                  {imagePreview ? (
                    ...
                    <div key={o._id} style={{ background: 'white', borderRadius: '12px', padding: '16px 20px', border: '1px solid #eef0f3' }}>
                      ...
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {Object.entries(STATUS_MAP).map(([key, val]) => (
                          <button
                            key={key}
                            onClick={() => updateOrderStatus(o._id, key)}
                            style={{
